...
export default function Home() {
  ...
  const speakName = (nameToSpeak: string) => {
    ...
    const utterance = new SpeechSynthesisUtterance(`Hello, ${nameToSpeak}!`);
    utterance.rate = 0.95;
    utterance.pitch = 1.1;

    // mouth animation shudhu actual speaking-er somoy cholbe
  ...
  return (
    <main className="min-h-screen bg-black text-white flex flex-col items-center justify-center gap-6 sm:gap-8 px-3 sm:px-6 py-8">
      ...
      {phase === "greeting" && (
        <div className="flex flex-col items-center gap-2 animate-character-in">
          <div className="relative bg-white text-black px-4 py-2 rounded-2xl rounded-bl-none text-sm sm:text-base font-medium shadow-lg">
            Hello, {committedName}! 👋
          </div>
